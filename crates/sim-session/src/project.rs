...
impl Project {
    ...
    #[must_use]
    pub fn capture_settings(state: &Session, theme: ThemeSpec, path: Option<&Path>) -> Self {
...
pub struct Restored {
    ...
    /// The traffic tabs the file described, not yet installed.
    ///
    /// Held back because a window drops the ones its dock has no tab for, and
    /// only a window has a dock. A front end without one installs them as they
    /// come, with [`Session::restore_monitors`].
...
pub struct UiSpec {
    ...
    #[serde(default, skip_serializing_if = "not_set")]
    pub hex_values: bool,
    /// The dock arrangement, as the window front end describes it.
    ///
    /// Carried rather than understood. Only a window has a dock, and a project
    /// saved from a terminal that dropped this section would come back with
    /// its panes rearranged.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn a_session_comes_back_the_way_it_was_written_down() {
        ...
        let written = Project::capture_settings(&state, ThemeSpec::Dark, None);
        ...
        assert_eq!(restored.theme, ThemeSpec::Dark);

        reopened.restore_monitors(restored.monitors);
        ...
        assert_eq!(
            Project::capture_settings(&reopened, restored.theme, None),
            written
        );
    ...
    fn an_untouched_traffic_tab_costs_no_lines() {
        let mut state = Session::default();
        state.open_monitor();
        let captured = Project::capture_settings(&state, ThemeSpec::Light, None);
        let text = toml::to_string_pretty(&captured).expect("should serialise");
    ...
    fn a_frames_folder_travels_with_the_file() {
        ...
        let project = Project::capture_settings(&state, ThemeSpec::Light, Some(&file));
...
"#,
    ...
    fn a_file_says_what_it_is_before_it_says_anything_else() {
        ...
        Project::capture_settings(&state, ThemeSpec::Light, Some(&file))
            .write(&file)
            .expect("should write");
